...
set "SCRIPT_DIR=%~dp0"
set "WEBSERVER=%SCRIPT_DIR%MAGNETO_Server.ps1"

if not exist "%WEBSERVER%" (
    echo [X] MAGNETO_Server.ps1 not found!
    echo [X] Expected location: %WEBSERVER%
    pause
    exit /b 1
)
echo [+] MAGNETO_Server.ps1 found.
...
powershell -ExecutionPolicy Bypass -File "%WEBSERVER%" -Port %PORT% -HostName %HOST%
